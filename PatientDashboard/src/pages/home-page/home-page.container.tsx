import * as React from "react";
import { RouteComponentProps } from "react-router";
import { HomePageComponent } from "./home-page.component";
import { searchPath } from "../search-page";
var qs= require('qs');

interface HomePageState {
  searchValue: string;
  identifier: string;
  patients: any
}

export class HomePageContainer extends React.Component<RouteComponentProps<any>, HomePageState> {
  constructor(props) {
    super(props);

    this.state = {
      searchValue: "",
      identifier: "-1",
      patients: []
    }
  }

  componentDidMount() {
    fetch("Fill in Patient API URL")
      .then(response => response.json())
      .then(data => {
        this.setState({ patients: data });
      });
  }
  
  private handleSearchSubmit = () => {        
    const params = qs.stringify({term: this.state.searchValue, identifier: this.state.identifier});

    this.props.history.push({
      pathname: searchPath,
      search: `?${params}`,
    });
    
  };

  private handleSearchUpdate = (newSearch: string) => {
    this.setState({...this.state, searchValue: newSearch});
  };

  handleChange = event => {
    this.setState({...this.state, identifier: event.target.value});
  };
  
  public render() {
    return (
      <HomePageComponent
        patients = {this.state.patients}
        selectedNihii= {this.state.identifier}
        searchValue={this.state.searchValue}
        onSearchSubmit={this.handleSearchSubmit}
        onSearchUpdate={this.handleSearchUpdate}
        handleChange = {this.handleChange}
      />
    )
  }
};
