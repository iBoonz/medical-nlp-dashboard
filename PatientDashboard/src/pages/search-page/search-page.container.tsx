...
class SearchPageInnerContainer extends React.Component<RouteComponentProps<any>, State> {
  ...
  componentDidMount() {
    if (isLastStateAvailable()) {
      ...
      this.handleReceivedSearchValue(receivedSearchValue.term, receivedSearchValue.identifier);
    }
  }

  // *** Search Value received through query string ***

  private handleReceivedSearchValue = (searchValue: string, identifier: string ) => {
    this.setState(
      receivedSearchValueUpdate(searchValue, identifier, true, "list"),
  ...
  private onGraphNodeDblClick = (value: string) => {
    ...
    this.handleReceivedSearchValue(searchValue, this.state.identifier);
  }
  ...
  private handleSearchSubmit = () => {
    this.setState(
      preSearchUpdate([{fieldId: "nihii", store: [this.state.identifier]}]),
