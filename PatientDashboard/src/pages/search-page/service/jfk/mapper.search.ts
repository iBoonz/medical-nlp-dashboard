...
const mapViewFilterToPayloadFilter = (filter: Filter): AzFilter => {
  console.log(filter)
  // TODO: This is just tailor made for JFK Demo event.
  if( filter.fieldId === "redactions"){
    return mapViewFilterToPayloadCustomFilter(filter)
  } else if (filter.fieldId === "nihii") {
    return {
      fieldName: "anonymizationId",
      operator: "eq",
      value: filter.store,
    }
  }else{
    return mapViewFilterToPayloadCollectionFilter(filter);
  }
}
